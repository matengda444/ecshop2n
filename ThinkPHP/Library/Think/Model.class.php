...
class Model {
     ...
     */
     //参数1：表单提交的数据
     //参数2：$type=1是执行插入操作， $type=2是执行更新操作，如果不传值，则是自己分析是插入还是更新。
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            //$data是表单接收字段  $fields是允许提交的字段。
            //如下foreach循环是过滤非法提交的字段。
            foreach ($data as $key=>$val){
                if(!in_array($key,$fields)) {
                    unset($data[$key]);
                }
            }
            //执行完成以上的foreach后，就把不允许提交的字段给过滤掉。
        }
